...
def djfrontend_normalize(v):
    ...
    """
    return '<link rel="stylesheet" href="%sskeleton/css/normalize/%s/normalize.css">' % (settings.STATIC_URL, v)
...
def djfrontend_h5bp_css(v):
    ...
    """
    return '<link rel="stylesheet" href="%sskeleton/css/h5bp/%s/h5bp.css">' % (settings.STATIC_URL, v)
...
def djfrontend_modernizr(v):
    ...
    if getattr(settings, 'TEMPLATE_DEBUG',):
        ...
        if hasattr(settings, 'DJFRONTEND_STATIC_URL'):
            return '<script src="%sskeleton/js/modernizr/%s/modernizr.min.js"></script>' % (settings.DJFRONTEND_STATIC_URL, v)
        else:
    ...
    if getattr(settings, 'TEMPLATE_DEBUG',):
        ...
        if hasattr(settings, 'DJFRONTEND_STATIC_URL'):
            output=[
                ...
                '<script>window.jQuery || document.write(\'<script src="%sskeleton/js/jquery/%s/jquery.min.js"><\/script>\')</script>' % (settings.DJFRONTEND_STATIC_URL, v)
            ]
        else:
            output=[
                '<script src="//ajax.googleapis.com/ajax/libs/jquery/%s/jquery.min.js"></script>' % v,
...
def twbs_css(v):
    ...
    """
    if getattr(settings, 'TEMPLATE_DEBUG',):
        return '<link rel="stylesheet" href="%sskeleton/css/twbs/%s/bootstrap.css">' % (settings.STATIC_URL, v)
    else:
        return '<link rel="stylesheet" href="%sskeleton/css/twbs/bootstrap.min.css">' % (settings.STATIC_URL, v)
...
def twbs_responsive_css(v):
    ...
    """
    if getattr(settings, 'TEMPLATE_DEBUG',):
        return '<link rel="stylesheet" href="%sskeleton/css/twbs/%s/bootstrap-responsive.css">' % (settings.STATIC_URL, v)
    else:
        return '<link rel="stylesheet" href="%sskeleton/css/twbs/%s/bootstrap-responsive.min.css">' % (settings.STATIC_URL, v)
...
SCRIPT_TAG = '<script src="%sskeleton/js/twbs/2.3.2/bootstrap-%s.js"></script>'
...
class BootstrapJSNode(template.Node):
    ...
    def render(self, context):
        if 'all' in self.args:
            if getattr(settings, 'TEMPLATE_DEBUG', ):
                return '<script src="%sskeleton/js/twbs/2.3.2/bootstrap.js"></script>' % settings.STATIC_URL
            else:
                if hasattr(settings, 'DJFRONTEND_STATIC_URL'):
                    return '<script src="%sskeleton/js/twbs/2.3.2/bootstrap.min.js"></script>' % settings.DJFRONTEND_STATIC_URL
                else:
                    return '<script src="%sskeleton/js/twbs/2.3.2/bootstrap.min.js"></script>' % settings.STATIC_URL
        else:
